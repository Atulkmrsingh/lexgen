# bash prepare_data.sh ../exp1 en hi /home/ganesh/udaan-indictrans/en-indic/
exp_dir=$1
src_lang=$2
tgt_lang=$3
...
if [[ "$vocab_type" == "sep" ]]
then
    bash apply_single_bpe_traindevtest_notag.sh $exp_dir $pre_trained_model_path
else 
    bash apply_bpe_traindevtest_notag.sh $exp_dir $pre_trained_model_path
fi
...
bash binarize_training_exp.sh $exp_dir SRC TGT $pre_trained_model_path/final_bin

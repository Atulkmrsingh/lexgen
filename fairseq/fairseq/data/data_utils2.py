...
def get_domain_list(): 
    #domain_dict,
    domain_tok = {}
    with open('/home/ganesh/lexgen/indicTrans/domain_list.txt') as f:
        for line in f.readlines():
            d = line.split(',')
            # domain_dict[d[0]] = d[1]
            domain_tok[int(d[0])] = d[1].strip()
    # print('domain tok is ', domain_tok)
    return domain_tok
...
def collate_tokens(
    ...
    domain_tok = {32062: '<admin>', 32063: '<agri>', 32064: '<bio>', 32065: '<biotech>', 32066: '<botany>', 32067: '<broadcast>', 32068: '<chem>', 32069: '<civil>', 32070: '<clinical>', 32071: '<comp>', 32072: '<electrical>', 32073: '<electronics>', 32074: '<geo>', 32075: '<history>', 32076: '<it>', 32077: '<math>', 32078: '<mech>', 32079: '<med>', 32080: '<microbio>', 32081: '<mineralogy>', 32082: '<mining>', 32083: '<nematology>', 32084: '<operational>', 32085: '<palaeobotany>', 32086: '<palaeontology>', 32087: '<petrology>', 32088: '<phy>', 32089: '<plasma>', 32090: '<political>', 32091: '<printing>', 32092: '<psychology>', 32093: '<quality>', 32094: '<rbi>', 32095: '<samaj>', 32096: '<sericulture>', 32097: '<shiksha>', 32098: '<sociology>', 32099: '<spacesci>', 32100: '<telecomm>', 32101: '<vanijya>', 32102: '<western>', 32103: '<zoology>'}
    num_domains = len(domain_tok)
    # print('domain tok', domain_tok)
    domain_matrix = torch.zeros(batch_size, dtype=torch.int32)#, num_domains))
    # print('batch batch_size inside datautisl ', batch_size)
    for i, v in enumerate(values):
        # print("v is",v)
        if key =='source':
            copy_tensor(v, res[i][size - len(v) :] if left_pad else res[i][: len(v)])
            # print(v)
            domain_matrix[i] = list(domain_tok.keys()).index(v[-2]) #check the index of domain tok with the src_tokens last index
        else:
            copy_tensor(v, res[i][size - len(v) :] if left_pad else res[i][: len(v)])
        
        
         # torch.nn.functional.one_hot(torch.tensor(index), num_domains).squeeze()
    # print('domain matrix ', domain_matrix)
    if key =='source':
        return res, domain_matrix
    else:
        return res
